...
describe('NWC Parser', () => {
	describe('decodeNwcArrayBuffer', () => {
		test('parses v1.55 file (jem001.nwc)', async () => {
		...
		test('parses v1.75 file (anongs.nwc)', async () => {
		...
		test('parses v1.75 file (adohn.nwc)', async () => {
		...
		test('parses v1.75 file (carenot.nwc)', async () => {
		...
		test('parses v1.75 file (bwv140-2.nwc)', async () => {
		...
		test('returns header with company and product', async () => {
			const contents = readFileSync('samples/carenot.nwc')
			const data = await decodeNwcArrayBuffer(contents)
		...
		test('returns score with staves array', async () => {
			const contents = readFileSync('samples/carenot.nwc')
			const data = await decodeNwcArrayBuffer(contents)
		...
		test('staves contain tokens', async () => {
			const contents = readFileSync('samples/carenot.nwc')
			const data = await decodeNwcArrayBuffer(contents)
		...
		test('tokens have type property', async () => {
			const contents = readFileSync('samples/carenot.nwc')
			const data = await decodeNwcArrayBuffer(contents)
