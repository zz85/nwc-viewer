...
- [x] **Cross-staff alignment with mismatched barlines** — when one staff has more barlines than another (e.g. mid-measure double barlines), notes at the same musical time must still align vertically and matching barlines must stack vertically. Fixed by: (1) removing `Barline` from `tabbableTypes` in the interpreter (barlines no longer advance `tabCounter`), (2) absorbing `BARLINE_NOTE_EXTRA` into the barline's own gap in the layout engine, (3) extracting `TickTracker` to `src/layout/tick-tracker.js` with dual-map alignment: `barlineTicks` stores pre-gap barline line positions for barline-to-barline vertical stacking, `maxTicks` stores post-gap positions for note alignment; barlines use `alignBarline()` (only consults `barlineTicks`), notes use `alignWithMax()` (consults `maxTicks` with `Math.max` to prevent backward snapping). Visual tests: "Matching Barlines Stack", "Extra Barline Notes Stack", "Multiple Extra Barlines Create Space" fixtures.
