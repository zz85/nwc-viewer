...
class ClefTxtObj extends NWCTxtObj {
  ...
  parse(fields) {
    ...
    for (const f of fields) {
      if (f.startsWith('Type:')) this.clefType = types[f.split(':')[1]] || 0;
...
class TimeSigTxtObj extends NWCTxtObj {
  constructor(s) { super(ObjType.TimeSig, s); this.beats = 4; this.beatValue = 4; }
  parse(fields) {
    for (const f of fields) {
      if (f.startsWith('Signature:')) {
        const sig = f.split(':')[1];
        if (sig === 'Common') { this.beats = 4; this.beatValue = 4; }
        else if (sig === 'AllaBreve') { this.beats = 2; this.beatValue = 2; }
        else {
...
class BarLineTxtObj extends NWCTxtObj {
  ...
  parse(fields) {
    ...
    for (const f of fields) {
      if (f.startsWith('Style:')) this.style = styles[f.split(':')[1]] || 0;
      if (f.startsWith('Repeat:')) this.repeatCount = parseInt(f.split(':')[1]) || 2;
    }
  }
  getStyle() { return this.style; }
  systemBreak() { return false; }
}
...
class RestTxtObj extends NWCTxtObj {
  ...
  parse(fields) {
    for (const f of fields) {
      ...
      if (f.startsWith('Offset:')) this.offset = parseInt(f.split(':')[1]) || 0;
    }
  }
  getDuration() { return this.duration; }
  getDurationType() {
...
class ChordTxtObj extends NWCTxtObj {
  ...
  parse(fields) {
    ...
    for (const f of fields) {
      ...
      if (f.startsWith('Pos:')) {
        for (const pos of f.split(':')[1].split(',')) {
          const m = pos.match(/([#bnxv]?)(-?\d+)/);
          if (m) {
            const acc = { '#': Accidental.Sharp, 'b': Accidental.Flat, 'n': Accidental.Natural, 'x': Accidental.SharpSharp, 'v': Accidental.FlatFlat };
    ...
    this.count = Math.max(0, positions.length - 1);
...
class TempoTxtObj extends NWCTxtObj {
  ...
  parse(fields) {
    for (const f of fields) {
      if (f.startsWith('Tempo:')) this.value = parseInt(f.split(':')[1]) || 120;
      ...
      if (f.startsWith('Text:')) this.text = f.split(':')[1].replace(/^"|"$/g, '');
    }
  }
...
class DynamicTxtObj extends NWCTxtObj {
  ...
  parse(fields) {
    ...
    for (const f of fields) {
      if (f.startsWith('Style:')) this.style = styles[f.split(':')[1]] ?? 4;
    }
  }
...
class FlowTxtObj extends NWCTxtObj {
  ...
  parse(fields) {
    ...
    for (const f of fields) {
      if (f.startsWith('Style:')) this.style = styles[f.split(':')[1].replace(/ /g, '')] ?? 0;
    }
  }
}
...
export function parseNWCTxt(text) {
  ...
  for (const line of text.split(/\r?\n/)) {
    ...
    if (line.startsWith('|')) {
      ...
      if (type === 'SongInfo') {
        for (const f of fields) {
          if (f.startsWith('Title:')) file.title = f.split(':')[1].replace(/^"|"$/g, '');
          if (f.startsWith('Author:')) file.author = f.split(':')[1].replace(/^"|"$/g, '');
          if (f.startsWith('Lyricist:')) file.lyricist = f.split(':')[1].replace(/^"|"$/g, '');
          if (f.startsWith('Copyright1:')) file.copyright1 = f.split(':')[1].replace(/^"|"$/g, '');
          if (f.startsWith('Copyright2:')) file.copyright2 = f.split(':')[1].replace(/^"|"$/g, '');
        }
      } else if (type === 'AddStaff') {
        staff = new NWCStaff(file);
        file.staffs.push(staff);
        for (const f of fields) {
          if (f.startsWith('Name:')) staff.name = f.split(':')[1].replace(/^"|"$/g, '');
          if (f.startsWith('Group:')) staff.group = f.split(':')[1].replace(/^"|"$/g, '');
        }
      } else if (type === 'StaffProperties' && staff) {
        for (const f of fields) {
          if (f.startsWith('Channel:')) staff.channel = parseInt(f.split(':')[1]) || 0;
        }
      } else if (type === 'StaffInstrument' && staff) {
        for (const f of fields) {
          if (f.startsWith('Patch:')) staff.patchName = parseInt(f.split(':')[1]) || 0;
        }
      } else if (staff) {
        let obj = null;
