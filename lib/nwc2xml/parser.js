...
function loadObject(r, staff) {
  // V205 uses 1-byte type + 1-byte skip, older versions use 2-byte type
  ...
  if (!obj) {
    console.warn(`Unknown object type: ${type} (0x${type.toString(16)}) at ${r.tell().toString(16)} - skipping`);
    // Try to skip unknown object by reading until next known type or reasonable boundary
    // For now, skip a reasonable amount of bytes
    const skipBytes = version >= NWC_Version.V205 ? 10 : 12;
    r.skip(skipBytes);
    return null;
  }
  obj.load(r);
