...
export function parseNWC(buffer) {
  ...
  console.log('NWC2XML Parser - Header:', header);
  
  if (header === NWZ_HEADER) {
    console.log('Decompressing NWZ file...');
    data = decompress(data.slice(6));
    console.log('Decompressed size:', data.length);
    // Debug: show first bytes at position 0x2a4
    console.log('Bytes at 0x2a4:', Array.from(data.slice(0x2a4, 0x2b0)).map(b => b.toString(16).padStart(2, '0')).join(' '));
  }
  ...
  console.log('File header check:', hdr.substring(0, 20) + '...');
...
function loadStaff(r, file) {
  ...
  r.skip(2); // style
  r.skip(2); // vertical size upper
  r.skip(2); // vertical size lower
  
  // V175+ adds lines (1 byte) after vertical size lower
  if (file.version >= NWC_Version.V175) r.skip(1);
  
  r.skip(2); // layer with next staff
  
  // V150+ adds transposition (2 bytes)
  if (file.version >= NWC_Version.V150) r.skip(2);
  ...
  console.log(`Staff ${staff.name}: objCount raw = ${objCount}, version = ${file.version.toString(16)}, pos = ${r.tell().toString(16)}`);
  
  // V200+ subtracts 2 from object count
  if (file.version >= NWC_Version.V200) objCount -= 2;
  console.log(`Staff ${staff.name}: objCount adjusted = ${objCount}`);
  ...
  if (file.version >= NWC_Version.V200 && !r.eof() && staff.objects.length > 0) {
...
function loadObject(r, staff) {
  // V170+ uses 2-byte type, but we need to check if it's actually 1-byte + skip
  // V205+ definitely uses 1-byte type + 1-byte skip
  const version = staff.file?.version || 0;
  const startPos = r.tell();
  
  // Try reading as 1-byte first for all versions
  const type = r.readUint8();
  const nextByte = r.readUint8();
  
  console.log(`loadObject at ${startPos.toString(16)}: type=${type} (0x${type.toString(16)}), nextByte=${nextByte}, version=${version.toString(16)}`);
  
  const obj = createObject(type, staff);
  if (!obj) {
    console.warn(`Unknown object type: ${type} (0x${type.toString(16)}) at ${startPos.toString(16)} - skipping`);
    // Skip a reasonable amount of bytes
    r.skip(10);
    return null;
  }
  obj.load(r);
